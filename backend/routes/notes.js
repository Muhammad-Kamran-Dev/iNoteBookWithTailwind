...
router.post('/addnote', fetchUser, [
    ...
    body("description", "Description must be atleast 5 character long").isLength({
        min: 5,
    }),
], async (req, res) => {
    ...
    try {
        ...
        const noteExist = await Note.findOne({ description,title});
        if (noteExist) {
            return res.status(500).json({ error: "Note already exists" })
        }
